<template>
    <div>
        <Modal v-model="modal.isGetAsset" :width="800" :mask-closable="false" title="资产申领">
            <Row class="row" :gutter="16">
                <Col span="12">
                    <Row>
                        <Col class="title" span="7">申领人:</Col>
                        <Col span="17">
                ...
                <Col span="12">
                    <Row>
                        <Col class="title" span="7">申领时间:</Col>
                        <Col span="17">
                            <DatePicker
                                type="date"
                                :value="new Date()"
            ...
            <div slot="footer">
                <Button type="text" size="large" @click="modal.isGetAsset=false">取消</Button>
...
<script>
    import { getAllStaffInfo, addGetAsset } from "api";
                                ...
                                on: {
                                    "on-blur": e => {
                                        ...
                                        if (!reg.test(e.target.value)) {
                                            ...
                                            this.$Message.error("申领数量不能大于资产数量");
        ...
        methods: {
            ...
            registerSubmitVal() {
                ...
                if (!this.form.applyPeople) {
                    this.$Message.warning("申领人不能为空！");
                    result = false;
                } else if (!this.form.applyTime) {
                    this.$Message.warning("请选择申领时间！");
                    result = false;
                } else if (!this.registerSl()) {
                    this.$Message.warning("申领的数量不能为空！");
                    result = false;
                }
                return result;
            },
            ...
            handleSubmit() {
                ...
                if (this.registerSubmitVal()) {
                    addGetAsset(this.getData()).then(res => {
                        console.log(res);
                        if (res.success) {
